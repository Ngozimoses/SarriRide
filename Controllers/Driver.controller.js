...
const DriverRefreshToken = async (req, res) => {
  try {
    ...
    }).limit(20); // safeguard: avoid scanning too many
    ...
    const driverCacheKey = `${CONFIG.REDIS_KEY_PREFIX}user:${tokenDoc.userId}:driver`;
    let driver = await redis.get(driverCacheKey);
    if (driver) {
      driver = JSON.parse(driver);
    } else {
      driver = await Driver.findOne({ _id: tokenDoc.userId, role: 'driver' }).lean();
      if (driver) {
        await redis.set(driverCacheKey, JSON.stringify(driver), 'EX', 3600);
      }
    }

    if (!driver) {
      logger.warn('Driver not found for refresh token', { driverId: tokenDoc.userId });
      return res.status(404).json({ status: 'error', message: 'Driver not found' });
    }

    //  Generate new tokens
    const newAccessToken = jwt.sign({ id: driver._id, role: driver.role }, process.env.JWT_SECRET, {
      expiresIn: CONFIG.JWT_ACCESS_TOKEN_EXPIRY,
    });
    ...
    try {
      await Promise.all([
        RefreshToken.updateOne(
          { _id: tokenDoc._id },
          { $set: { revoked: true, revokedAt: new Date(), replacedByTokenId: newTokenDoc._id } }
        ),
        newTokenDoc.save(),
        redis.set(
          `${CONFIG.REDIS_KEY_PREFIX}blacklist:${tokenDoc._id}`,
          'revoked',
          'EX',
          Math.floor((tokenDoc.expiresAt - Date.now()) / 1000)
        ),
      ]);
    } catch (redisError) {
      logger.error('Redis error during token refresh', { error: redisError.message });
      throw redisError; // Let outer catch handle rollback
    }

    const encryptedNewAccessToken = encrypt(newAccessToken);
    const encryptedNewRefreshToken = encrypt(newRefreshToken);

    logger.info('Driver access token refreshed', { driverId: driver._id, email: driver.email });
    return res.json({
      status: 'success',
...
module.exports = { verifyEmail, verifyDriverOtp, registerDriver, uploadImages, DriverLogin , DriverRefreshToken};
